...
router = APIRouter(prefix="/test-runs", tags=["Test Runs"], dependencies=[Depends(verify_token)])

@router.get("", response_model=list[TestRunResponse])
def get_test_runs(status: Optional[str] = None, test_type: Optional[str] = None, sort: Optional[str] = None, db: Session = Depends(get_db)):
...
def create_test_run(test_run: TestRunCreate, db: Session = Depends(get_db)):
    new_test_run = TestRun(**test_run.model_dump())
    db.add(new_test_run)
    db.commit()
...
def delete_all_test_runs(db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"message": "All test runs deleted", "deleted_count": deleted_count}
